...
class optimizer(Node):
    ...
    def trajectory_info_callback(self,msg):
        if self.trajectory_type_valid is False:
            ...
            self.get_logger().info(f"grads: {optimizer.get_future_reward_grad( jnp.array([0.38514861, 0.68848354, -0.6646899, 0.07905411, -0.31466046, 0.02618345]).reshape(-1,1), jnp.array([7.0, 4.0]), jnp.array([45.46159775]) )}")
    ...
    def optimize(init_state, deltaT, params_policy):
            ...
            jax.debug.print("init state is: {x} at time: {t}", x=init_state, t=deltaT)
