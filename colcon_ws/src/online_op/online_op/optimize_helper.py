...
@jit
def find_ref_pos_vel_acc(trajectory_type, deltaT, parameters):
    radius, angular_vel, center_x, center_y, height = parameters

    def circle_case(deltaT, radius, angular_vel, center_x, center_y, height):
        return circle_pos_vel_acc(deltaT, radius, angular_vel, center_x, center_y, height)

    def figure8_case(deltaT, radius, angular_vel, center_x, center_y, height):
        return figure8_pos_vel_acc(deltaT, radius, angular_vel, center_x, center_y, height)

    ref_pos, ref_vel, ref_acc = lax.cond(trajectory_type == 0,
                                         circle_case,
                                         figure8_case,
                                         deltaT, radius, angular_vel, center_x, center_y, height)
